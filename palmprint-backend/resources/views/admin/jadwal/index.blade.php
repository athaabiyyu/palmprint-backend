...
const hariBadge = { senin:'primary', selasa:'success', rabu:'warning', kamis:'danger', jumat:'info' };
...
async function loadDropdowns() {
    const [resSemester, resProdi, resKelas, resMatkul, resDosen] = await Promise.all([
...
// ── Saat Prodi dipilih di modal → filter Kelas & Matkul ──
function onProdiChange() {
    ...
    const filteredMatkul = prodiId
        ? allMatkuls.filter(m => String(m.prodi_id) === String(prodiId))
        : allMatkuls;
    matkulSelect.innerHTML = '<option value="">-- Pilih Mata Kuliah --</option>';
    filteredMatkul.forEach(m => {
...
async function loadData() {
    ...
    tbody.innerHTML = data.map((j, i) => `
        <tr>
            <td>${i + 1}</td>
            <td><small class="text-muted">${j.semester ? j.semester.nama : '-'}</small></td>
            ...
            <td><i class="bi bi-clock me-1"></i>${j.jam_mulai} - ${j.jam_selesai}</td>
            <td>${j.ruangan ?? '-'}</td>
            <td>
                <button class="btn btn-warning btn-sm me-1"
                    onclick="edit(${j.id}, ${j.semester_id}, ${j.kelas_id}, ${j.mata_kuliah_id},
                        ${j.dosen_id}, '${j.hari}', '${j.jam_mulai}', '${j.jam_selesai}',
                        '${j.ruangan ?? ''}', ${j.kelas?.prodi_id ?? 'null'})">
    ...
    // Update filter kelas berdasarkan prodi yang dipilih
...
function showModal() {
    ...
    document.getElementById('kelasId').innerHTML    = '<option value="">-- Pilih Kelas --</option>';
    document.getElementById('matkulId').innerHTML   = '<option value="">-- Pilih Mata Kuliah --</option>';
...
// ── Show Modal Edit ──
function edit(id, semesterId, kelasId, matkulId, dosenId, hari, jamMulai, jamSelesai, ruangan, prodiId) {
    ...
    // Filter kelas & matkul berdasarkan prodi
    onProdiChange();
...
async function simpan() {
    ...
    if (!data.semester_id || !data.kelas_id || !data.mata_kuliah_id ||
        !data.dosen_id || !data.jam_mulai || !data.jam_selesai) {
